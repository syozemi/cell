...
def train(seed):
    ...
    image, mask, num_mask = pro.load_unet_n_data(seed,mode=0)
    ...
    torch.save(net, 'model/unet_n/%d' % seed)

    pro.make_dir('log')
    pro.make_dir('log/unet_n')

    pro.save(log, 'log/unet_n', str(seed))

    end_time = time.time()
    time_taken = (end_time - start_time) // 60

    print('saved model as model/unet_n/%d' % seed)
    print('took %d minutes' % time_taken)


def view(seed):
    image, mask = pro.load_unet_n_data(seed,mode=2)
    n = int(len(image) // 4)
    for i in range(n):
        start = i * 4
        img = image[start:start+4]
        msk = mask[start:start+4]
        net = torch.load('model/unet_n/%d' % seed)
        net.cuda()
...
if __name__ == '__main__':
    pro.make_dir('model/unet_n')
    files = os.listdir('model/unet_n')
    seed = len(files)
    train(seed)
    view(seed)